...
bool ImplicitFunction::runOnModule(Module &M) {
    ...
    for (const_inst_iterator I = inst_begin(*F); I != inst_end(*F); ++I) {
      if (const CallInst *callInst = dyn_cast<CallInst>(&*I)) {
        ...
        if (tainted.find(parent) != tainted.end()) {
          if (const DILocation *loc = callInst->getDebugLoc()) {
  ...
  for (Module::const_iterator F = M.begin(); F != M.end(); ++F) {
    for (auto &bb : *F) {
      ...
      if (auto term = bb.getTerminator()) {
        isBranching = term->getNumSuccessors() > 1;
        if (auto br = dyn_cast<const BranchInst>(term))
          if (br->isConditional() &&
              tainted.find(br->getCondition()) != tainted.end())
            isTainted = true;
      ...
      std::string nodeName =
          (bb.getParent()->getName() + ": " + bb.getName()).str();

      for (auto succ : successors(&bb)) {
        errs() << "[EDGE]\"" << nodeName << "\" -> \""
               << bb.getParent()->getName() << ": " << succ->getName()
               << "\"\n";
      }
          ...
          if (tainted.find(parent) != tainted.end()) {
            if (const DILocation *loc = callInst->getDebugLoc()) {
              ...
              if (callee) {
                ...
                if (print) {
                  // errs() << "[EDGE]\"" << bb.getParent()->getName() << ": "
                  //        << bb.getName() << "_" << count++ << "\" -> \""
                  //        << callee->getName() << ": call_site" <<
                  //        loc->getLine()
                  //        << "\"\n";
                  std::string calleeName = (callee->getName() + ": call_site" +
                                            std::to_string(loc->getLine()))
                                               .str();
                  errs() << "[NODE]\"" << calleeName << "\"\n";
                  errs() << "[EDGE]\"" << nodeName << "\" -> \"" << calleeName
                         << "\"\n";
                  errs() << "[EDGE]\"" << calleeName << "\" -> \""
                         << callee->getName() + ": entry"
                         << "\"\n";
                }
              }
            }
      ...
      std::string color = isTainted ? "color=\"red\"" : "";
      std::string br = isBranching ? "comment=\"condbr\"" : "";
      errs() << "[NODE]\"" << nodeName << "\" ";
      // errs() << " [ " << color << " " << br << " ]\n";
      if (color.size() > 0 || br.size() > 0) {
        errs() << " [" << color;
        if (color.size() > 0 && br.size() > 0)
          errs() << "  ";
        errs() << br << "]\n";
