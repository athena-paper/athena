...
const DILocation *Infoflow::findVar(const Value *V, const Function *F) const {
...
const DILocalVariable *Infoflow::findVarNode(const Value *V,
...
void Infoflow::getOriginalLocation(const Value *V) {
  ...
  if (const Instruction *I = dyn_cast<Instruction>(V)) {
    Loc = I->getDebugLoc();
    if (!Loc) {
      const DILocalVariable *lv = findVarNode(V, I->getParent()->getParent());
      if (!lv) {
        DEBUG_WITH_TYPE(DEBUG_TYPE_DEBUG, errs()
    ...
    Loc = findVar(V, F);
    if (!Loc) {
      const DILocalVariable *lv = findVarNode(V, F);
      if (!lv) {
...
/// Helper function that computes the join of all operands to an instruction and
/// the pc, and then makes the result of the instruction at least as high.
...
int Infoflow::matchValueAndParsedString(const Value &value, std::string kind,
  ...
  if (var.function.size() == 0 ||
    ...
    const DILocalVariable *local_var;
    if (fn) {
